...
CHECK1		CMP		R6,#0
			...
			BNE		NOSPEC1
			MOV		R2,#0xFF000000
			ADD		R2,R2,#0x00C00000
			B		DONE
NOSPEC1		MOVNE	R2,#0x0	;this wasn't infinity times 0 which is a special case with 0xFFC00000 output
			BNE		SIGNING
CHECK2		CMP		R7,#0
			BNE		CHECK3
			CMP		R10,#0
...
ADDITEXPON	STMED	R13!,{R0,R1,R4-R5,R7-R12,LR}
			MOV		R0,#0xFF
			ADD		R0,R0,#0x100
...
LOOP2		LSRS		R3,R3,#1
			...
			ADD		R12,R12,#1
			CMP		R12,#22
			BNE		LOOP2 ;shifts 23 times to the right
			...
			MOVNE	R6,#1
			LSRNE	R2,R2,#1
			LDMED	R13!,{R0,R1,R4-R5,R7-R12,PC}
